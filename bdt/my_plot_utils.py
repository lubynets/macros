...
def plot_feature_imp(df_in, y_truth, model, labels=None, n_sample=10000, approximate=True):
    ...
    """
    print("my_plot_utils::plot_feature_imp(): 0")
    class_labels, class_counts = np.unique(y_truth, return_counts=True)
    n_classes = len(class_labels)
    print("my_plot_utils::plot_feature_imp(): 1")
    for class_count in class_counts:
        n_sample = min(n_sample, class_count)

    subs = []
    print("my_plot_utils::plot_feature_imp(): 2")
    for class_lab in class_labels:
    ...
    res = []
    print("my_plot_utils::plot_feature_imp(): 3")
    if n_classes <= 2:
        print("my_plot_utils::plot_feature_imp(): n_classes <= 2")
        res.append(plt.figure(figsize=(18, 9)))
        print("my_plot_utils::plot_feature_imp(): 311")
        shap.summary_plot(shap_values, df_subs, plot_size=(
            18, 9), class_names=labels, show=False)
        print("my_plot_utils::plot_feature_imp(): 312")
    else:
        print("my_plot_utils::plot_feature_imp(): n_classes > 2")
        for i_class in range(n_classes):
            print("my_plot_utils::plot_feature_imp(): 321")
            res.append(plt.figure(figsize=(18, 9)))
            print("my_plot_utils::plot_feature_imp(): 322")
            shap_values_class = shap_values[i_class]
            print("my_plot_utils::plot_feature_imp(): 3222")
            print(f'shap_values_class.shape[0] == {shap_values_class.shape[0]}')
            print(f'df_subs.shape[0] == {df_subs.shape[0]}')
            assert shap_values_class.shape[0] == df_subs.shape[0], "Mismatch between SHAP values and DataFrame rows"
            shap.summary_plot(shap_values_class, df_subs, plot_size=(
                18, 9), class_names=labels, show=False)
            print("my_plot_utils::plot_feature_imp(): 323")
    print("my_plot_utils::plot_feature_imp(): 4")
    res.append(plt.figure(figsize=(18, 9)))
    shap.summary_plot(shap_values, df_subs, plot_type='bar', plot_size=(
        18, 9), class_names=labels, show=False)
    print("my_plot_utils::plot_feature_imp(): 5")
    return res
