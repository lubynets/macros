...
void tree_qa(const std::string& fileName = "/home/oleksii/alidir/sandbox/AnalysisResults_trees.nolite.root", int selectionFlag=1) {
      ...
      if(std::abs(flag) == LcFlag) { // is signal
        if(swapped == 0) { // PKPi is true signal
          if(isSelPKPi >= selectionFlag) {
            ...
            else                       hmass.at(kNonPrompt)->Fill(massPKPi);
          }
          ...
          if(isSelPiKP >= selectionFlag) {
            ...
            else                       hmass.at(kNonPrompt)->Fill(massPiKP);
          }
...
void CheckTreesNEntriesConsistensy(std::vector<TTree*> trees) {
  ...
  if(!(std::equal(Nentries.begin() + 1, Nentries.end(), Nentries.begin()))) {
    throw std::runtime_error("CheckTreesNEntriesConsistensy() faild!");
  }
}
