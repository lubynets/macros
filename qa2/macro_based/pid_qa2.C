...
const std::vector<std::string> PtTypes{"PtProng", "PtMother"};

void pid_qa2(const std::string& fileName) {
  TFile* fileIn = TFile::Open(fileName.c_str(), "read");
  for(const auto& ptType : PtTypes) {
    for(int iPs=0, nPss=ProngSpecies.size(); iPs<nPss; iPs++) {
      for(int iDet=0, nDets=PidDetectors.size(); iDet<nDets; iDet++) {
        TH2* h = fileIn->Get<TH2>(("PidQa/h2nSig" + PidDetectors.at(iDet) + ProngSpecies.at(iPs) + "Vs" + ptType).c_str());
        TCanvas cc("cc", "");
        ...
        cc.Print(("pid_qa." + ptType + ".pdf" + priBra).c_str(), "pdf");
      } // nPidDetectors
    } // nProngSpecies
  } // PtTypes
  fileIn->Close();
