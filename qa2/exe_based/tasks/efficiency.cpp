...
//  }
  ...
  for(auto& var : variables) {
    ...
    if(isSaveToRoot) {
      fileOut->cd();
      for(auto& histo : std::vector<TH1*>{histoEffPrompt, histoEffNonPrompt, histoRelErrEffPrompt, histoRelErrEffNonPrompt, histoRecPrompt, histoRecNonPrompt, histoGenPrompt, histoGenNonPrompt}) {
        histo->Write();
      }
    }
